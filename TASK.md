...
**修正方針:**
- 中央カラムに名前空間矩形を配置し、同じ名前空間内のクラス矩形は中央カラム最大幅まで横並びにする
- 名前空間が変わる場合、または同じ名前空間内で中央カラム最大幅を超える場合に新しいクラス行へ折り返す
...
- [x] 既存の自由配置・直交グリッド探索ルーティングを削除し、固定レーン方式へ置き換える
- [ ] クラス・名前空間の行ベース配置を、同一名前空間内の横並び行へ修正する
- [x] 各行に必要な水平レーン数を計算し、行上部余白へ反映する
- [x] 依存エッジごとに水平レーン・サイドレーンを割り当てる
- [x] 上辺接続点の均等配置を実装する
- [x] 同一行・近接行の矢印を行上部水平レーンだけで描画する
- [x] 離れた行の矢印を左右サイドレーン経由で描画する
- [x] 診断JSONを固定レーン方式の row / horizontalLane / sideLane 情報に更新する
- [ ] DebugProject と大規模プロジェクトで描画速度・矩形貫通なしを確認する

### 12.8 上辺接続方向ルールの明確化

**問題:** 下の行にある source から上の行にある target へ向かう近接行ルートで、target 上辺への最後の縦線が下から上方向になり、矢印記号が上向きに描画される。

**修正方針:**
- 始点は下から上方向、終点は上から下方向で接続することを仕様に明記する
- 同一行・近接行ルートでも、target 上辺へ入る最後の縦線が必ず上から下方向になる水平レーンを選択する

- [x] specification.md に上辺接続方向ルールを追記
- [x] 同一行・近接行ルートの水平レーン選択を修正

### 12.9 除外パターンのワイルドカード対応

**問題:** `UnityEngine` の完全一致だけでは `UnityEngine.InputSystem` などの子名前空間をまとめて除外できない。

**修正方針:**
- 末尾が `.*` の除外パターンを、名前空間・フォルダ名の前方一致として扱う
- 通常パターンは従来どおり完全一致とする
- 完全除外と内部依存除外の両方に一致する場合は、完全除外を優先する仕様として明記する

- [x] `UnityEngine.*` / `Unity.*` 形式の前方一致を実装
- [x] 設定画面の説明文を更新
- [x] specification.md / architecture.md に一致仕様と優先関係を追記

### 12.10 除外パターン追加時のフォーカス改善

**問題:** 設定画面で `+` ボタンを押して行を追加したとき、追加行の入力欄にフォーカスされず、スクロール外に追加された場合に編集しづらい。

**修正方針:**
- 行追加後に追加された入力欄へフォーカスする
- 追加行がスクロール外の場合は画面内へスクロールする

- [x] 追加された除外パターン項目をViewModelから通知する
- [x] 設定画面で追加行のTextBoxへスクロール・フォーカスする
- [x] 追加行の下半分が見切れないよう、行全体が表示されるスクロール位置へ調整する

### 12.11 アプリケーションアイコンの設定

**問題:** タスクバーに表示したときにアイコンが設定されておらず、他のアプリと見分けにくい。

**修正方針:**
- `Dog` から `Animal` へ下向きの実装矢印を示すアイコンを作成する
- EXE のアプリケーションアイコンと WPF ウィンドウアイコンに同じ `.ico` を設定する

- [x] `Dog` / 下向き実装矢印 / `Animal` の `.ico` を追加
- [x] EXE とメインウィンドウにアイコンを設定
